...
export async function updateUserRole(userId: string, role: ROLE) {
    ...
    if (!session?.user?.role || !["Vlasnik", "Programer"].includes(session.user.role))
...
export async function updateProfile(data: {
    ...
    if (!success) return { error: "Previše pokušaja. Pokušajte ponovo za 15 minuta." }

    const user = await prisma.user.findUnique({ where: { id: session.user.id } })
    if (!user) return { error: 'Korisnik nije pronađen' }

    const passwordMatch = await bcryptjs.compare(data.currentPassword, user.password)
    ...
    if (data.newPassword) updateData.password = await bcryptjs.hash(data.newPassword, 10)
...
export async function registerUser(email: string, password: string, name: string) {
    ...
    if (!name || !email || !password) return { error: "Morate popuniti sva polja." }
    ...
    try {
        ...
        if (existingUser) return { error: "Korisnik sa ovim email-om već postoji." }
