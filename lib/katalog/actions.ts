...
export async function createNarudzba(data: {
    ...
    if (!data.ime?.trim() || !data.prezime?.trim()) return { error: "Ime i prezime su obavezni." }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) return { error: "Nevažeći email." }
    if (!/^\+?[\d\s\-()]{6,20}$/.test(data.telefon)) return { error: "Nevažeći telefon." }
