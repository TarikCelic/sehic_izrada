...
export async function updateOrderStatus(orderId: string, status: string) {
  ...
  if (
    !session?.user?.role ||
    !["Vlasnik", "Programer", "Moderator"].includes(session.user.role)
  ) {
...
export async function createProduct(formData: FormData) {
  ...
  if (!ime || !opis || !dimenzije)
    return { error: "Sva polja moraju biti popunjena!" };
  ...
  if (savedPicsUrls.length < 1)
    return { error: "Morate dodati bar jednu sliku!" };
