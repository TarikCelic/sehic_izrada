...
const Page = async () => {
    ...
    const top5Prodanih = top5ProdanihRaw.map((item) => ({
        name:
            top5ProductNames.find((p) => p.id === item.productId)?.name ??
            "Nepoznat",
    ...
    return (
        <section className="flex flex-col gap-8 p-4 sm:p-6">
            <div className="animate-fadeUp">
                ...
                <p className="text-sm text-amber-600 mt-0.5">Sve informacije na jednom mjestu.</p>
            </div>
                ...
                {dashboardStats.map((item, index) => (
                    ...
                    >
                        <div className="flex justify-between items-center">
                            ...
                            <p className="text-(--primary) font-bold text-xl">{item.number}</p>
                        </div>
            ...
            <div className="flex flex-col w-full h-full rounded-2xl overflow-hidden border border-gray-100 animate-fade">
                ...
                {products.map((item: Product) => (
                    ...
                    >
                        <div className="w-20 shrink-0 text-gray-400">#{item.id.slice(-6)}</div>
                ...
                <div className="bg-white p-6 rounded-sm border border-gray-100 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {top5Prodanih.length === 0 ? (
                            <p className="text-xs text-gray-400 text-center py-4">Nema završenih narudžbi</p>
                        ) : (
                ...
                <div className="bg-white p-6 rounded-sm border border-gray-100 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {top5Posjeta.length === 0 ? (
                            <p className="text-xs text-gray-400 text-center py-4">Nema podataka o posjetama</p>
                        ) : (
                            top5Posjeta.map((item, i) => {
                                ...
                                const pct = Math.round(((item.visitors ?? 0) / maxPosjeta) * 100);
                                return (
                                    <div key={i} className="space-y-1">
                                        <div className="flex justify-between text-xs font-medium">
                                            <span className="truncate pr-2">{item.name}</span>
                                            <span className="shrink-0">{item.visitors ?? 0} klikova</span>
                                        </div>
                                        <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
                                            <div
                                                className={`${barColors[i]} h-full transition-all duration-500`}
