...
export default async function KatalogPage({ searchParams }: PageProps) {
    ...
    const categories = params.categories ? (params.categories.split(",").filter(Boolean) as Category[]) : [];
    const woodTypes  = params.woodTypes  ? (params.woodTypes.split(",").filter(Boolean)  as WoodType[]) : [];
    const colors     = params.colors     ? (params.colors.split(",").filter(Boolean)     as Color[])    : [];
                ...
                <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6">
                    {products.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 gap-4 text-white/20">
                            ...
                            <p className="text-sm font-medium">Pokušaj s drugačijim filterima</p>
                        </div>
                    ) : (
