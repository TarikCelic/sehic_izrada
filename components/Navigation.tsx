...
export default function Navigation() {
    ...
    const isOBP =  pathname.startsWith("/about") || pathname.startsWith("/kontakt")
    const doWhite = pathname.startsWith("/register") || pathname.startsWith("/login") || pathname.startsWith("/account") || pathname.startsWith("/favourites");
    ...
    return (
        <>
            <SearchInput isOpen={searchActive} onClose={() => setSearchActive(false)} />
            <nav
            ...
            >
                ...
                >
                    <span className={`w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out ${
                        isMobileOpen ? 'rotate-45 translate-y-2 bg-black' : (isLightMode ? 'bg-black' : 'bg-white')
                    }`} />
                    <span className={`w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out ${
                        isMobileOpen ? 'opacity-0 scale-0' : (isLightMode ? 'bg-black' : 'bg-white')
                    }`} />
                    <span className={`w-6 h-0.5 rounded-full transition-all duration-300 ease-in-out ${
                        isMobileOpen ? '-rotate-45 -translate-y-2 bg-black' : (isLightMode ? 'bg-black' : 'bg-white')
                    }`} />
            ...
            >
                <div className="flex flex-col h-full pt-6 px-6 pb-8">
                    ...
                    <div className="mt-auto border-t border-gray-200 pt-6 space-y-4">
                        <button
                            onClick={() => { closeMenu(); toggleSearch(); }}
