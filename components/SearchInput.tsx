...
const SearchInput = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    ...
    const showEmpty = status === 'done' && query.length >= 2 && results.length === 0
    const showResults = status === 'done' && results.length > 0

    return (
        <div ref={containerRef} className={`p-4 fixed top-0 w-full left-0 z-[100] bg-white flex flex-col px-6 lg:px-22 py-10 shadow-lg transition-all duration-300 ${
            ...
            <div className={"flex items-center gap-4"}>
                <div className="border border-black/20 flex gap-3 w-full p-4 items-center focus-within:border-(--primary) transition-colors duration-200">
                    ...
                    <button
                        onClick={() => { setQuery(''); setResults([]); setStatus('idle') }}
                ...
                <div onClick={onClose} className={"flex justify-center items-center p-4 w-15 h-15 text-2xl font-mono cursor-pointer hover:border-[#FFFFFFAA] hover:bg-(--primary) hover:text-white transition-all duration-200 border border-black/20"}>
                    X
                </div>
            </div>
            ...
            <div className="h-6 mt-2 flex items-center">
                {status === 'loading' && (
                    <p className="text-xs text-gray-400 animate-pulse">Pretraživanje u tijeku...</p>
                )}
                {showResults && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 overflow-y-auto max-h-[70vh] pt-4">
                    ...
                    results.map((e, i) => (
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-gray-600 font-medium">{e.price.toLocaleString()} KM</p>
                            </div>
                        </Link>
                    ))
                }
                ...
                {showEmpty && (
                    <p className="col-span-full text-center py-10 text-gray-500">
                        Nismo pronašli ništa za <span className="font-medium">"{query}"</span>
                    </p>
                )}
            </div>
        </div>
