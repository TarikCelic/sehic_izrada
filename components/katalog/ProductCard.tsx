      ...
      <div className="p-4 flex flex-col gap-1">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3 mb-2">
            {hasWhite && (
              <div
                className={`w-4 h-4 rounded-full bg-white outline ${isFavourites ? "outline-black/10" : "outline-white/20"} outline-offset-2`}
                title="Bijela"
              />
            )}
            {hasBlack && (
              <div
                className={`w-4 h-4 rounded-full bg-black outline ${isFavourites ? "outline-black/10" : "outline-white/20"} outline-offset-2`}
                title="Crna"
              />
            )}
            {hasWooden && (
              <div
                className={`w-4 h-4 rounded-full bg-[#8B4513] outline ${isFavourites ? "outline-black/10" : "outline-white/20"} outline-offset-2`}
                title="Drvo"
              />
            )}
          </div>
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <span
            className={`text-xs ${textSecondary} border rounded-md px-2 py-1 ${borderStyle} lowercase first-letter:uppercase`}
          >
            {product.category}
          </span>
        </div>
