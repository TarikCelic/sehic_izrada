...
function Section({
  ...
  return (
    <div className="pb-5 border-b border-white/8 last:border-0 last:pb-0">
      <p className="text-[9px] font-bold tracking-[0.2em] uppercase text-white/30 mb-3 pl-1">
        {title}
      </p>
      {children}
    </div>
...
function FilterPanel({ onClose }: { onClose?: () => void }) {
  ...
  const categories = useMemo(
    () =>
      (sp.get("categories")?.split(",").filter(Boolean) ?? []) as Category[],
    [sp],
  );
  const woodTypes = useMemo(
    () => (sp.get("woodTypes")?.split(",").filter(Boolean) ?? []) as WoodType[],
    [sp],
  );
  const colors = useMemo(
    () => (sp.get("colors")?.split(",").filter(Boolean) ?? []) as Color[],
    [sp],
  );
  ...
  const setCategories = (next: Category[]) =>
    push({ categories: next.join(",") || null });
  const setWoodTypes = (next: WoodType[]) =>
    push({ woodTypes: next.join(",") || null });
  const setColors = (next: Color[]) => push({ colors: next.join(",") || null });
  const setSort = (val: SortBy) =>
    push({ sortBy: val === "newest" ? null : val });
        ...
        <div className="flex flex-col gap-0.5">
          {CATEGORIES.map((cat) => {
            ...
            return (
              ...
              >
                <span
                  className={`transition-colors ${active ? "text-[var(--primary)]" : "text-white/30 group-hover:text-white/55"}`}
                >
                  {Icon}
                </span>
        ...
        <div className="px-1">
          <div className="flex justify-between items-baseline mb-3">
            <span className="text-sm font-semibold text-[var(--primary)]">
              {localMin.toLocaleString()} KM
            </span>
            <span className="text-xs text-white/35">—</span>
            <span className="text-sm font-semibold text-white/70">
              {localMax.toLocaleString()} KM
            </span>
          </div>
...
export default function ProductFilters() {
  ...
  const activeCount = useMemo(() => {
    ...
    const price =
      Number(sp.get("minPrice") ?? 0) > 0 ||
      Number(sp.get("maxPrice") ?? MAX_PRICE) < MAX_PRICE
        ? 1
        : 0;
