...
export default function NaruciProizvod({
  productId,
  productName,
  availableColors,
  dimensions,
  ...
  const inputClass =
    "w-full border-2 border-white/10 bg-white/5 text-white placeholder:text-white/30 p-3 outline-none focus:border-[var(--primary)] transition-all duration-200 rounded-lg text-sm";
  const labelClass =
    "text-[10px] text-white/40 uppercase tracking-widest mb-1 block";

  const modal = (
    <div
      className={`fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity duration-200 ${isClosing || !isVisible ? "opacity-0" : "opacity-100"}`}
    >
      <div
        className={`bg-[var(--foreground)] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto flex flex-col  transition-all duration-200 ease-in-out
    ${
      isClosing || !isVisible
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => set("postanskiBroj", e.target.value)}
                      placeholder="71000"
                      className={inputClass}
                    />
                  </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-3 mt-1">
                    {availableColors.map((color) => (
                      ...
                      >
                        <span
                          className={`w-4 h-4 rounded-full border ${colorStyles[color]}`}
                        />
                        {colorLabels[color]}
                      </button>
                    ))}
                ...
                <div>
                  ...
                  <p className="text-white/30 text-xs mt-1">
                    Standardne dimenzije: {dimensions}. Možete prilagoditi.
                  </p>
                </div>
