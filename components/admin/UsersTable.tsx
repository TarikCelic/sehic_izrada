    ...
    return (
        <>
            <div className="hidden sm:block bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
                <table className="w-full text-sm animate-fade">
                    ...
                    <tr className="bg-zinc-50 border-b border-zinc-200">
                        {["ID", "PREVIEW", "IME", "EMAIL", "ULOGA", "NAPRAVLJEN", "AKCIJE"].map((h) => (
                            <th
                                key={h}
                    ...
                    {users.map((user, i) => (
                        ...
                        >
                            <td className=" py-3.5 text-center border-r border-zinc-100">
                                <span className="text-amber-500 font-semibold">#{user.id}</span>
                            </td>

                            <td className="px-4 py-3.5 text-center border-r border-zinc-100">
                                {user.image ? (
                                    <img src={user.image} alt="" className="w-8 h-8 rounded-full object-cover mx-auto" />
                                ) : (
                  ...
                  <span className="text-zinc-400 text-xs">
                    ...
                    {users.length === 0 && (
                        <tr>
                            <td colSpan={7} className="py-14 text-center text-zinc-300 text-sm">
                                Nema korisnika.
                            </td>
                        </tr>
                    )}
            ...
            <div className="sm:hidden space-y-3 animate-fade-down">
                {users.map((user, i) => (
                    <div key={user.id} className="bg-white border border-zinc-200 rounded-xl p-4 shadow-sm ">
                        <div className="flex items-center gap-3 mb-3">
                            <span className="text-amber-500 font-bold text-sm w-7">#{i + 1}</span>
                            {user.image ? (
