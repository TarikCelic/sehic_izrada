...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${cls}`}
    >
      {status}
    </span>
  );
        ...
        <table className="w-full text-sm animate-fade-down">
          <thead>
            <tr className="bg-zinc-50 border-b border-zinc-200">
              {["ID", "NARUČUJE", "PROIZVOD", "ADRESA", "STATUS", "AKCIJE"].map(
                (h) => (
                  <th
                    key={h}
                    className="px-4 py-3 text-center text-[11px] font-semibold text-zinc-400 tracking-widest uppercase border-r border-zinc-100 last:border-r-0"
                  >
                    {h}
                  </th>
                ),
